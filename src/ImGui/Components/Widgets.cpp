...
bool ImGui::DragFloat3(const char *label, glm::vec3 &value, float reset_value,
  ...
  { /* Reset Button X */
    ...
    if (ButtonEx("X", buttonSize, 0, ImDrawFlags_RoundCornersLeft))
      value.x = reset_value;
    PopStyleColor(3);
  }
  SameLine();
  ...
  SameLine();
  { /* Drag Z */
    DragScalar("##Z", ImGuiDataType_Float, &value.z, 0.1f, NULL, NULL, NULL, 0,
               ImDrawFlags_RoundCornersRight);
  }
  PopItemWidth();
  PopStyleVar();
